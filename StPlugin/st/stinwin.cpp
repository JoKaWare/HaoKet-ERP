...
StInWin::StInWin(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::StInWin)
{
    ui->setupUi(this);
...
StInWin::StInWin(QString no,QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::StInWin)
{
    ui->setupUi(this);
...
StInWin::StInWin(int rdf, QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::StInWin)
{
    ui->setupUi(this);
...
StInWin::StInWin(int rdf, QString no,QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::StInWin)
{
    ui->setupUi(this);
...
void StInWin::readAuthority(){
    LimitsCommon::authority(ui->toolBar,mWinId.toInt());
}

void StInWin::init(){
    //窗体ID号
    mWinId = HaoKeApp::stSubId() + StPlugin::otherInWinId();
    mWinTitle = "其他入库单";

    //01 其他入库单 查看vouch_type表
    mVouchType = HaoKeApp::getVouchRdOtherIn();
    mVouchSource = "";
    ui->rdLabel->setText("");
    if(mRdFlag == RdFlag::RdOutFlag){
        ...
        ui->label_16->setText("<html><head/><body><p><span style=\" color:#ff0000;\">其他入库单</span></p></body></html>");
    }
    ...
    actionProperties actsBase[]= {
        { "stRdIn.new",tr("&New"),SLOT(sNew()), NULL, "New", QPixmap(":/image/Add.png"), NULL,  true, tr("New") },
        { "stRdIn.edit",tr("&Edit"),SLOT(sEdit()), NULL, "Edit", QPixmap(":/image/edit.png"), NULL,  true, tr("Edit") },
        { "stRdIn.delete",tr("&Delete"),SLOT(sDelete()), NULL, "Delete", QPixmap(":/image/cross_48.png"), NULL,  true, tr("Delete") },
        { "stRdIn.save",tr("&Save"),SLOT(sSave()), NULL, "Save", QPixmap(":/image/save.png"), NULL,  true, tr("Save") },
        { "stRdIn.audit",tr("&Audit"),SLOT(sAudit()), NULL, "Audit", QPixmap(":/image/stamp_48.png"), NULL,  true, tr("Audit") },
        { "stRdIn.abandonAudit",tr("&Abandon Audit"),SLOT(sAbAudit()), NULL, "Abandon Audit", QPixmap(":/image/c_stamp_48.png"), NULL,  true, tr("Abandon Audit") },
        { "stRdIn.delRow",tr("&DelRow"),SLOT(sDelRow()), NULL, "DelRow", QPixmap(":/image/minus.png"), NULL,  true, tr("DelRow") },
        { "stRdIn.print",tr("&Print"),SLOT(sPrint()), NULL, "Print", QPixmap(":/image/printer.png"), NULL,  true, tr("Print") },
        { "stRdIn.first",tr("&First"),SLOT(sFirst()), NULL, "First", QPixmap(":/image/first48.png"), NULL,  true, tr("First") },
        { "stRdIn.next",tr("&Next"),SLOT(sNext()), NULL, "Next", QPixmap(":/image/n-next48.png"), NULL,  true, tr("Next") },
        { "stRdIn.previous",tr("&Previous"),SLOT(sPrevious()), NULL, "Previous", QPixmap(":/image/n-previous48.png"), NULL,  true, tr("Previous") },
        { "stRdIn.end",tr("&End"),SLOT(sEnd()), NULL, "End", QPixmap(":/image/end48.png"), NULL,  true, tr("End") },
        { "stRdIn.cancel",tr("&Cancel"),SLOT(sCancel()), NULL, "Cancel", QPixmap(":/image/cancel.png"), NULL,  true, tr("Cancel") }
...
void StInWin::footResized(int logicalIndex, int oldSize, int newSize){
    mFootView->resizeSection(logicalIndex + 1,newSize);
    mFootView->resizeSection(0,ui->tableWidget->verticalHeader()->width());
}
void StInWin::refreshData(){
...
void StInWin::inputClear(){
    WidgetBeanUtil::clearLayoutWidgetData(ui->headGridLayout);


}

void StInWin::setInputEnable(bool b){
...
void StInWin::addActionsTo(actionProperties acts[], unsigned int numElems){
...
void StInWin::toolBarActTriggered(QAction *act){
    //写入操作日志
...
void StInWin::sDelete(){
    ...
    if(QMessageBox::warning(this,"提示","是否删除？",QMessageBox::Yes | QMessageBox::No)==QMessageBox::Yes){
        ...
        if(bean->busType()!="调拨入库"){
            int imeiCount = RdDAO::getRdInImeiCount(ui->noEdit->text());
            if(imeiCount > 0){
...
void StInWin::setValue(int &recNo, QString &paramName, QVariant &paramValue, int reportPage){
    Q_UNUSED(reportPage);
...
void StInWin::sCancel(){

    this->close();
}

void StInWin::closeEvent(QCloseEvent *event){
...
void StInWin::on_tableWidget_itemChanged(QTableWidgetItem *item)
{
