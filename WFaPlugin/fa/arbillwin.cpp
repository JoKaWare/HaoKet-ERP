...
ArBillWin::ArBillWin(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::ArBillWin)
{
    ui->setupUi(this);
    ...
    setupContextMenu();
    mCountRecord = ApCloseBillDAO::getApCloseBillCount(RdFlag::RdInFlag) - 1;
    if(mCountRecord < 0) mCountRecord = 0;
    mRecord = mCountRecord;
    QList<ApCloseBillBean *> vouchBeanList = ApCloseBillDAO::queryApCloseBillByPage(mCountRecord,1,RdFlag::RdInFlag);
...
ArBillWin::ArBillWin(QString no, QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::ArBillWin)
{
    ui->setupUi(this);
...
void ArBillWin::readAuthority(){
    LimitsCommon::authority(ui->toolBar,mWinId.toInt());
}

void ArBillWin::init(){
    //窗体ID号
    mWinId = HaoKeApp::faSubId() + WFaPlugin::arBillWinId();
    mWinTitle = "收款单";
    ...
    actionProperties actsBase[]= {
        { "faArBill.new",tr("&New"),SLOT(sNew()), NULL, "New", QPixmap(":/image/Add.png"), NULL,  true, tr("New") },
        { "faArBill.edit",tr("&Edit"),SLOT(sEdit()), NULL, "Edit", QPixmap(":/image/edit.png"), NULL,  true, tr("Edit") },
        { "faArBill.delete",tr("&Delete"),SLOT(sDelete()), NULL, "Delete", QPixmap(":/image/cross_48.png"), NULL,  true, tr("Delete") },
        { "faArBill.save",tr("&Save"),SLOT(sSave()), NULL, "Save", QPixmap(":/image/save.png"), NULL,  true, tr("Save") },
        { "faArBill.audit",tr("&Audit"),SLOT(sAudit()), NULL, "Audit", QPixmap(":/image/stamp_48.png"), NULL,  true, tr("Audit") },
        { "faArBill.abandonAudit",tr("&Abandon Audit"),SLOT(sAbAudit()), NULL, "Abandon Audit", QPixmap(":/image/c_stamp_48.png"), NULL,  true, tr("Abandon Audit") },
        { "faArBill.delRow",tr("&DelRow"),SLOT(sDelRow()), NULL, "DelRow", QPixmap(":/image/minus.png"), NULL,  true, tr("DelRow") },
        { "faArBill.print",tr("&Print"),SLOT(sPrint()), NULL, "Print", QPixmap(":/image/printer.png"), NULL,  true, tr("Print") },
        { "faArBill.first",tr("&First"),SLOT(sFirst()), NULL, "First", QPixmap(":/image/first48.png"), NULL,  true, tr("First") },
        { "faArBill.next",tr("&Next"),SLOT(sNext()), NULL, "Next", QPixmap(":/image/n-next48.png"), NULL,  true, tr("Next") },
        { "faArBill.previous",tr("&Previous"),SLOT(sPrevious()), NULL, "Previous", QPixmap(":/image/n-previous48.png"), NULL,  true, tr("Previous") },
        { "faArBill.end",tr("&End"),SLOT(sEnd()), NULL, "End", QPixmap(":/image/end48.png"), NULL,  true, tr("End") },
        { "faArBill.cancel",tr("&Cancel"),SLOT(sCancel()), NULL, "Cancel", QPixmap(":/image/cancel.png"), NULL,  true, tr("Cancel") }
    ...
    childItemProperties childItem[] = {
            ...
            { "amount","收款金额","收款金额", 3,80, true, true },
    ...
    */
    ...
    QList<ApTypeBean *> apTypeList = KmCodeDAO::queryApType(RdFlag::RdInFlag);
    for(int i = 0 ; i < apTypeList.size() ; i ++ ){
        busTypeList << apTypeList.at(i)->typeName();
    }
...
void ArBillWin::footResized(int logicalIndex, int oldSize, int newSize){
    mFootView->resizeSection(logicalIndex + 1,newSize);
}

void ArBillWin::refreshData(){
...
void ArBillWin::read(QString no){
    ApCloseBillBean *bean = ApCloseBillDAO::getApCloseBillByNo(no);
    mNo = no;
    if(bean != NULL && bean->apNo().length() > 0){
        ...
        CustomerBean *cusBean = CustomerDAO::getCusByCode(bean->busCode());
        ui->busQsk->setText(bean->busCode() + " - " + cusBean->cusName());
...
void ArBillWin::inputClear(){
    WidgetBeanUtil::clearLayoutWidgetData(ui->headGridLayout);


}

void ArBillWin::setInputEnable(bool b){
...
void ArBillWin::addActionsTo(actionProperties acts[], unsigned int numElems){
...
void ArBillWin::sFirst(){
    ...
    refreshData();
    QList<ApCloseBillBean *> vouchBeanList = ApCloseBillDAO::queryApCloseBillByPage(0,1,RdFlag::RdInFlag);
    if(vouchBeanList.size()>0){
        read(vouchBeanList.at(0)->apNo());
...
void ArBillWin::sNext(){
    ...
    ui->tableWidget->clear();
    refreshData();
    QList<ApCloseBillBean *> vouchBeanList = ApCloseBillDAO::queryApCloseBillByPage(mRecord,1,RdFlag::RdInFlag);
    if(vouchBeanList.size()>0){
        read(vouchBeanList.at(0)->apNo());
    }
...
void ArBillWin::sNew(){
    ...
    mNo = ItemUtil::getToDayMaxNo(HaoKeApp::faSubId() + WFaPlugin::apBillWinId(),"ap_close_bill",QDate::currentDate(),nCount);
...
void ArBillWin::sSave(){
    ...
    if(mEditStatus == addFlag){
        ...
        //收款单
        apBillBean->setBusFlag(RdFlag::RdInFlag);
...
void ArBillWin::sPrint(){

    QString fileName = "ReportfaArBill.xml";
...
void ArBillWin::setValue(int &recNo, QString &paramName, QVariant &paramValue, int reportPage){
    Q_UNUSED(reportPage);
...
void ArBillWin::sCancel(){

    this->close();
}

void ArBillWin::closeEvent(QCloseEvent *event){
...
void ArBillWin::setupContextMenu(){
    /*
...
void ArBillWin::on_tableWidget_itemChanged(QTableWidgetItem *item)
{
...
void ArBillWin::on_busQsk_editChanged(const QString &arg1)
{
    CustomerBean *sBean = CustomerDAO::getCusByCode(arg1);
    if(sBean && sBean->cusCode().length() > 1){
        ui->bankNameEdit->setText(sBean->cusBank());
        ui->bankCardEdit->setText(sBean->cusAccount());
    }
}
