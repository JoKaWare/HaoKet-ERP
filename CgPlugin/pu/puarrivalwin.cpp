                ...
                if(oBean && oBean->orderNo().length() > 2){
                    ...
                    if(real - puVouchsBean->quantity() < 0){
                        QMessageBox::warning(this,"提示","订单:"+ puVouchsBean->orderNo() + " ,商品："
                                             + puVouchsBean->productName() + " 可到货数量不足！,最大到货数量为:" + QString::number(real));
                        return ;
                    }
                ...
                if(oBean && oBean->orderNo().length() > 2){
                    ...
                    if(real - mChildVouchsList.at(i)->quantity() < 0){
                        QMessageBox::warning(this,"提示","订单:"+ mChildVouchsList.at(i)->orderNo() + " ,商品："
                                             + mChildVouchsList.at(i)->productName() + " 可到货数量不足！,最大到货数量为:" + QString::number(real));
                        return ;
                    }
                ...
                if(oBean && oBean->orderNo().length() > 2){
                    ...
                    if(real - mAddChildVouchsList.at(i)->quantity() < 0){
                        QMessageBox::warning(this,"提示","订单:"+ mAddChildVouchsList.at(i)->orderNo() + " ,商品："
                                             + mAddChildVouchsList.at(i)->productName() + " 可到货数量不足！,最大到货数量为:" + QString::number(real));
                        return ;
                    }
    ...
    if(bean && bean->no().length() > 0){
            ...
            for(int i=0;i<vouchsList.size();i++){
                ...
                if(oBean && oBean->orderNo().length() > 2){
                    ...
                    if(real - vouchsList.at(i)->quantity() < 0){
                        QMessageBox::critical(this,"错误","订单:" + vouchsList.at(i)->orderNo() + " 商品：" + vouchsList.at(i)->productName() + " 可到货数量为：" + QString::number(real) );
                        return ;
                    }
                }else{
        ...
        if(mIsMakeRd){
            ...
            if(ui->memoEdit->text().length() < 1)
                rdbean->setMemo("到货单自动生成,单号:" + bean->no());
            else
        ...
        if(mIsHathOrder){
            for(int i=0;i<vouchsList.size();i++){
                ...
                if(oBean && oBean->orderNo().length() > 2){
                    ...
                    if(real - vouchsList.at(i)->quantity() < 0){
                        QMessageBox::critical(this,"错误","订单:" + vouchsList.at(i)->orderNo() + " 商品："
                                              + vouchsList.at(i)->productName() + " 可到货数量为：" + QString::number(real) );
                        return ;
                    }
                }else{
                    QMessageBox::critical(this,"错误","订单:" + vouchsList.at(i)->orderNo() + " 商品："
...
void PuArrivalWin::sChoose(){
    ...
    cVouchDlg->setWindowTitle("提取采购订单");
    ...
    childItemProperties childItem[] = {
            ...
            { "not_quantity","未到货数量","未到货数量", 10, 80,true, true },
