...
PuReturnedQueryForm::PuReturnedQueryForm(QWidget *parent) :
    QueryForm(parent)
{
    QString lSql1 = "select e.*,d.emp_name from (select f.*,c.dep_name from "
            ...
            " WHERE a.bus_type='02' AND a.bus_date>=? AND a.bus_date<=? AND b.supplie_type like ? AND a.supplier_id like ? ) f "
    ...
    childItemProperties listItem[] = {
            ...
            { "supplier_id","编码","供应商", 2,80, true, false },
            { "supplie_name","名称","供应商", 3, 120,true, true },
            { "dep_name","部门","部门", 4,80, true, false },
            { "emp_name","采购员","采购员", 5,60, true, false },
    ...
    QString dSql1 =
            " SELECT a.id, a.no, a.bus_date, a.bus_type, a.pu_type, a.supplier_id, b.supplie_name, a.wh_code,a.dep_code,c.order_no,c.product_code,"
                       ...
                       " WHERE a.bus_type='02' AND a.bus_date>=? AND a.bus_date<=? AND b.supplie_type like ? AND a.supplier_id like ? "
    ...
    QString dSql2 = "select f.*,b.supplie_name from ( "
            ...
            "  WHERE a.no like ? AND a.bus_type='02' AND a.no=c.arrival_no AND c.product_code=d.product_code) f "
            " left join ba_suppliers b on f.supplier_id= b.supplie_code ORDER BY id ASC";
    childItemProperties childItem[] = {
            { "no","到货单号","到货单号", 0,100, true, true },
            { "bus_date","到货日期","到货日期", 1,80, true, true },
            { "supplier_id","编码","供应商", 2,60, true, false },
            { "supplie_name","名称","供应商", 3, 120,true, true },
            { "order_no","订单号","订单号", 4,100, true, true },
            ...
            { "quantity","退货数量","退货数量", 9, 60,true, true },
            { "price","采购单价","采购单价", 10,80, true, true },
            { "money","采购金额","采购金额", 11,80, true, false }
    ...
    QString formId = HaoKeApp::puSubId() + CgPlugin::returnedQueryWinId();
    QString winTitle = "采购退货单查询";
    HaoKeApp::writeSysLog(HaoKeApp::getInstance()->getAccountId() ,winTitle,HaoKeApp::puSubId().toInt(),formId.toInt(),0,"");

    setFormId(formId);
    setTabListText("采购退货单列表");
    setTabDetailText("采购退货明细");
    //设置明细合计列号
    QList<int > totalItem;
    totalItem << 9 << 11;
...
void PuReturnedQueryForm::sTableClicked(QString no){
    if(no.length()>1){
        if(!HaoKeApp::getInstance()->gui()->subWinIsOpen(tr("Purchase Returned"))){
            PuReturnedWin *pWin = new PuReturnedWin(no,this);
