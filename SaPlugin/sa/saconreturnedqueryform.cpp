#include "saconreturnedqueryform.h"
...
SaConReturnedQueryForm::SaConReturnedQueryForm(QWidget *parent) :
    QueryForm(parent)
{
    QString lSql1 = "select e.*,d.emp_name from (select f.*,c.dep_name from "
            ...
            " WHERE a.bus_type='04' AND a.bus_date>=? AND a.bus_date<=? AND b.cus_type like ? AND a.cus_code like ? ) f "
    ...
    QString lSql2 = "select e.*,d.emp_name from (select f.*,c.dep_name from "
            " (select a.*,b.cus_name from sa_dispatch_vouch a "
            " left join ba_customer b on a.cus_code=b.cus_code "
            " WHERE a.no like ? AND a.bus_type='04'  ) f"
    ...
    childItemProperties listItem[] = {
            ...
            { "sa_type","委托代销类型","委托代销类型", 2,80, true, false },
    ...
    QString dSql1 = "select f.*,j.dep_name,h.emp_name,w.wh_name from ( "
            " SELECT a.id, a.no, a.bus_date, a.bus_type, a.sa_type, a.cus_code, b.cus_name, a.wh_code,a.dep_code,c.order_no,c.product_code,"
                       ...
                       " WHERE a.bus_type='04' AND a.bus_date>=? AND a.bus_date<=? AND b.cus_type like ? AND a.cus_code like ? "
    ...
    QString dSql2 = "select f.*,b.cus_name,j.dep_name,h.emp_name,w.wh_name from ( "
            ...
            "  WHERE a.no like ? AND a.bus_type='04' AND a.no=c.dispatch_no AND c.product_code=d.product_code) f "
    ...
    childItemProperties childItem[] = {
            ...
            { "unit","单位","单位", 12,60, true, false },
            { "quantity","数量","数量", 13, 60,true, true },
            { "price","单价","单价", 14,80, true, true },
            { "money","代销金额","代销金额", 15,80, true, false }
...
void SaConReturnedQueryForm::sTableClicked(QString no){
    if(no.length()>1){
        if(!HaoKeApp::getInstance()->gui()->subWinIsOpen(tr("Consignment Returned"))){
            SaConReturnedWin *pWin = new SaConReturnedWin(no,this);
