...
SaReturnedWin::SaReturnedWin(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::SaReturnedWin)
{
    ui->setupUi(this);
    init();
    //qApp->setStyle("Plastique");
    setupContextMenu();
    mCountRecord = SaDispatchDAO::getSaDispatchCount("02") - 1;
    mRecord = mCountRecord;
    if(mCountRecord < 0) mCountRecord = 0;
    QList<SaDispatchVouchBean *> vouchBeanList = SaDispatchDAO::querySaDispatchByPage(mCountRecord,1,"02");
...
SaReturnedWin::SaReturnedWin(QString no, QWidget *parent):
    QMainWindow(parent),
    ui(new Ui::SaReturnedWin)
{
    ui->setupUi(this);
...
SaReturnedWin::~SaReturnedWin()
{
    delete ui;
}

void SaReturnedWin::readAuthority(){
    LimitsCommon::authority(ui->toolBar,mWinId.toInt());
}
void SaReturnedWin::init(){
    QApplication::setOverrideCursor(Qt::WaitCursor);
    //窗体ID号
    mWinId = HaoKeApp::saSubId() + SaPlugin::returnedWinId();
    mWinTitle = "销售退货单";
    ...
    actionProperties actsBase[]= {
        { "saReturned.new",tr("&New"),SLOT(sNew()), NULL, "New", QPixmap(":/image/Add.png"), NULL,  true, tr("New") },
        { "saReturned.edit",tr("&Edit"),SLOT(sEdit()), NULL, "Edit", QPixmap(":/image/edit.png"), NULL,  true, tr("Edit") },
        { "saReturned.delete",tr("&Delete"),SLOT(sDelete()), NULL, "Delete", QPixmap(":/image/cross_48.png"), NULL,  true, tr("Delete") },
        { "saReturned.save",tr("&Save"),SLOT(sSave()), NULL, "Save", QPixmap(":/image/save.png"), NULL,  true, tr("Save") },
        { "saReturned.audit",tr("&Audit"),SLOT(sAudit()), NULL, "Audit", QPixmap(":/image/stamp_48.png"), NULL,  true, tr("Audit") },
        { "saReturned.abandonAudit",tr("&Abandon Audit"),SLOT(sAbAudit()), NULL, "Abandon Audit", QPixmap(":/image/c_stamp_48.png"), NULL,  true, tr("Abandon Audit") },
        { "saReturned.delRow",tr("&DelRow"),SLOT(sDelRow()), NULL, "DelRow", QPixmap(":/image/minus.png"), NULL,  true, tr("DelRow") },
        { "saReturned.print",tr("&Print"),SLOT(sPrint()), NULL, "Print", QPixmap(":/image/printer.png"), NULL,  true, tr("Print") },
        { "saReturned.choose",tr("&Choose"),SLOT(sChoose()), NULL, "Choose", QPixmap(":/image/page_accept.png"), NULL,  true, tr("Choose") },
        { "saReturned.first",tr("&First"),SLOT(sFirst()), NULL, "First", QPixmap(":/image/first48.png"), NULL,  true, tr("First") },
        { "saReturned.next",tr("&Next"),SLOT(sNext()), NULL, "Next", QPixmap(":/image/n-next48.png"), NULL,  true, tr("Next") },
        { "saReturned.previous",tr("&Previous"),SLOT(sPrevious()), NULL, "Previous", QPixmap(":/image/n-previous48.png"), NULL,  true, tr("Previous") },
        { "saReturned.end",tr("&End"),SLOT(sEnd()), NULL, "End", QPixmap(":/image/end48.png"), NULL,  true, tr("End") },
        { "saReturned.cancel",tr("&Cancel"),SLOT(sCancel()), NULL, "Cancel", QPixmap(":/image/cancel.png"), NULL,  true, tr("Cancel") }
...
void SaReturnedWin::footResized(int logicalIndex, int oldSize, int newSize){
    mFootView->resizeSection(logicalIndex + 1,newSize);
}
void SaReturnedWin::refreshData(){
...
void SaReturnedWin::read(QString no){
    SaDispatchVouchBean *bean = SaDispatchDAO::getSaDispatchByNo(no);
    mNo = no;
    if(bean != NULL && bean->no().length() > 0){
...
void SaReturnedWin::inputClear(){
    WidgetBeanUtil::clearLayoutWidgetData(ui->headGridLayout);


}

void SaReturnedWin::setInputEnable(bool b){
...
void SaReturnedWin::addActionsTo(actionProperties acts[], unsigned int numElems){
...
void SaReturnedWin::sFirst(){
    ...
    refreshData();
    QList<SaDispatchVouchBean *> vouchBeanList = SaDispatchDAO::querySaDispatchByPage(0,1,"02");
    if(vouchBeanList.size()>0){
        read(vouchBeanList.at(0)->no());
...
void SaReturnedWin::sNext(){
    ...
    ui->tableWidget->clear();
    refreshData();
    QList<SaDispatchVouchBean *> vouchBeanList = SaDispatchDAO::querySaDispatchByPage(mRecord,1,"02");
    if(vouchBeanList.size()>0){
        read(vouchBeanList.at(0)->no());
    }
...
void SaReturnedWin::sCancel(){

    this->close();
}

void SaReturnedWin::closeEvent(QCloseEvent *event){
...
void SaReturnedWin::setupContextMenu(){
    setContextMenuPolicy(Qt::ActionsContextMenu);
}

void SaReturnedWin::sNew(){
    inputClear();
    mIsEdit = true;
    mEditStatus = addFlag;
    setInputEnable(true);
...
void SaReturnedWin::sSave(){
    ...
    if(mEditStatus == addFlag){
        ...
        for(int i = 0;i < rowNum; i++){
            ...
            SaDispatchVouchsBean *oBean = SaDispatchDAO::getSaReRdsBy(puVouchsBean->srcNo(),puVouchsBean->productCode());
        ...
        for(int i = 0; i < mChildVouchsList.size(); i++){
            SaDispatchVouchsBean *oBean = SaDispatchDAO::getSaReRdsBy(mChildVouchsList.at(i)->srcNo(),mChildVouchsList.at(i)->productCode());
            if(oBean && oBean->srcNo().length() > 2){
                double real = oBean->quantity();
        ...
        for(int i = 0; i < mAddChildVouchsList.size(); i++){
            SaDispatchVouchsBean *oBean = SaDispatchDAO::getSaReRdsBy(mAddChildVouchsList.at(i)->srcNo(),mAddChildVouchsList.at(i)->productCode());
            if(oBean && oBean->srcNo().length() > 2){
                double real = oBean->quantity();
...
void SaReturnedWin::sAudit(){
    ...
    if(bean && bean->no().length() > 0){
        ...
        if(mIsMakeRd){
            ...
            rdbean->setVouchType(HaoKeApp::getVouchRdSaRe());
            ...
            if(ui->memoEdit->text().length() < 1)
                rdbean->setMemo("退货单自动生成,单号:" + bean->no());
...
void SaReturnedWin::sPrint(){
    QString fileName = "ReportsaReturned.xml";
    QString title = "销售退货单";
    QString formatData = "";
...
void SaReturnedWin::setValue(int &recNo, QString &paramName, QVariant &paramValue, int reportPage){
    Q_UNUSED(reportPage);
...
void SaReturnedWin::on_tableWidget_itemChanged(QTableWidgetItem *item)
{
    if(item->column() == 0 && mIsEdit){
        ...
        if(mIsHathOrder && !mIsChoose){
            QMessageBox::warning(this,"提示","退货请提取出库单，不可直接录入商品！");
            ui->tableWidget->model()->setData(ui->tableWidget->model()->index(item->row(),0),"");
            return ;
        }
        //根据ID查询原料信息
