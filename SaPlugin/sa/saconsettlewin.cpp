#include "saconsettlewin.h"
#include "ui_saconsettlewin.h"
#include "haokeapp.h"
...
SaConSettleWin::SaConSettleWin(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::SaConSettleWin)
{
    ui->setupUi(this);
    init();
    //qApp->setStyle("Plastique");
    setupContextMenu();
    mCountRecord = SaSettleDAO::getSaSettleCount("02") - 1;
    mRecord = mCountRecord;
    if(mCountRecord < 0) mCountRecord = 0;
    QList<SaSettleVouchBean *> vouchBeanList = SaSettleDAO::querySaSettleByPage(mCountRecord,1,"02");
...
SaConSettleWin::SaConSettleWin(QString no,QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::SaConSettleWin)
{
    ui->setupUi(this);
...
SaConSettleWin::~SaConSettleWin()
{
    delete ui;
}

void SaConSettleWin::readAuthority(){
    LimitsCommon::authority(ui->toolBar,mWinId.toInt());
}

void SaConSettleWin::init(){

    //窗体ID号
    mWinId = HaoKeApp::saSubId() + SaPlugin::conSettleWinId();
    mWinTitle = "委托代销结算单";
...
void SaConSettleWin::footResized(int logicalIndex, int oldSize, int newSize){
    mFootView->resizeSection(logicalIndex + 1,newSize);
}

void SaConSettleWin::refreshData(){
...
void SaConSettleWin::read(QString no){
    SaSettleVouchBean *bean = SaSettleDAO::getSaSettleByNo(no);
    mNo = no;
    if(bean != NULL && bean->no().length() > 0){
...
void SaConSettleWin::inputClear(){
    WidgetBeanUtil::clearLayoutWidgetData(ui->headGridLayout);


}

void SaConSettleWin::setInputEnable(bool b){
...
void SaConSettleWin::addActionsTo(actionProperties acts[], unsigned int numElems){
...
void SaConSettleWin::sFirst(){
    ...
    refreshData();
    QList<SaSettleVouchBean *> vouchBeanList = SaSettleDAO::querySaSettleByPage(0,1,"02");
    if(vouchBeanList.size()>0){
        read(vouchBeanList.at(0)->no());
...
void SaConSettleWin::sNext(){
    ...
    ui->tableWidget->clear();
    refreshData();
    QList<SaSettleVouchBean *> vouchBeanList = SaSettleDAO::querySaSettleByPage(mRecord,1,"02");
    if(vouchBeanList.size()>0){
        read(vouchBeanList.at(0)->no());
    }
...
void SaConSettleWin::sCancel(){

    this->close();
}

void SaConSettleWin::closeEvent(QCloseEvent *event){
...
void SaConSettleWin::setupContextMenu(){
    setContextMenuPolicy(Qt::ActionsContextMenu);
}

void SaConSettleWin::sNew(){
    inputClear();
    mIsEdit = true;
    mEditStatus = addFlag;
    setInputEnable(true);
    ...
    mNo = ItemUtil::getToDayMaxNo(HaoKeApp::saSubId() + SaPlugin::settleWinId(),"sa_settle_vouch",QDate::currentDate(),nCount);
...
void SaConSettleWin::sPrint(){
    QString fileName = "ReportSaSettle.xml";
    QString title = "委托代销结算单";
    QString formatData = "";
...
void SaConSettleWin::sChoose(){
    if(ui->cusQks->text().trimmed().length() < 1){
        QMessageBox::warning(this,"提示","请输入客户");
        return ;
    }
    ChooseVouchDialog *cVouchDlg = new ChooseVouchDialog(this);
    cVouchDlg->setWindowTitle("提取委托代销出库单");
    ...
    if(ui->busTypeComboBox->currentText() == "红字结算"){
        sql = " SELECT a.id, a.rd_no, a.bus_date, a.bus_type, a.cus_code, b.cus_name, a.wh_code,a.dep_code,c.product_code,"
                        ...
                        " AND d.type_id like ? AND c.product_code like ?"
                        " AND a.rd_no=c.rd_no AND a.is_handle=true AND c.product_code=d.product_code AND a.cus_code= b.cus_code "
                        " AND a.vouch_type='12' ORDER BY id ASC";
    }
...
void SaConSettleWin::setValue(int &recNo, QString &paramName, QVariant &paramValue, int reportPage){
    Q_UNUSED(reportPage);
...
void SaConSettleWin::on_tableWidget_itemChanged(QTableWidgetItem *item)
{
