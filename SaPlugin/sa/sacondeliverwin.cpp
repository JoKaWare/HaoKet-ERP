#include "sacondeliverwin.h"
#include "ui_sacondeliverwin.h"
#include "haokeapp.h"
...
#include "saplugin.h"
SaConDeliverWin::SaConDeliverWin(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::SaConDeliverWin)
{
    ui->setupUi(this);
    init();
    //qApp->setStyle("Plastique");
    setupContextMenu();
    mCountRecord = SaDispatchDAO::getSaDispatchCount("03") - 1;
    mRecord = mCountRecord;
    if(mCountRecord < 0) mCountRecord = 0;
    QList<SaDispatchVouchBean *> vouchBeanList = SaDispatchDAO::querySaDispatchByPage(mCountRecord,1,"03");
...
SaConDeliverWin::SaConDeliverWin(QString no, QWidget *parent):
    QMainWindow(parent),
    ui(new Ui::SaConDeliverWin)
{
    ui->setupUi(this);
...
SaConDeliverWin::~SaConDeliverWin()
{
    delete ui;
}

void SaConDeliverWin::readAuthority(){
    LimitsCommon::authority(ui->toolBar,mWinId.toInt());
}
void SaConDeliverWin::init(){
    QApplication::setOverrideCursor(Qt::WaitCursor);
    //窗体ID号
    mWinId = HaoKeApp::saSubId() + SaPlugin::conDeliverylWinId();
    mWinTitle = "委托代销发货单";
    ...
    actionProperties actsBase[]= {
        { "saConDelivery.new",tr("&New"),SLOT(sNew()), NULL, "New", QPixmap(":/image/Add.png"), NULL,  true, tr("New") },
        { "saConDelivery.edit",tr("&Edit"),SLOT(sEdit()), NULL, "Edit", QPixmap(":/image/edit.png"), NULL,  true, tr("Edit") },
        { "saConDelivery.delete",tr("&Delete"),SLOT(sDelete()), NULL, "Delete", QPixmap(":/image/cross_48.png"), NULL,  true, tr("Delete") },
        { "saConDelivery.save",tr("&Save"),SLOT(sSave()), NULL, "Save", QPixmap(":/image/save.png"), NULL,  true, tr("Save") },
        { "saConDelivery.audit",tr("&Audit"),SLOT(sAudit()), NULL, "Audit", QPixmap(":/image/stamp_48.png"), NULL,  true, tr("Audit") },
        { "saConDelivery.abandonAudit",tr("&Abandon Audit"),SLOT(sAbAudit()), NULL, "Abandon Audit", QPixmap(":/image/c_stamp_48.png"), NULL,  true, tr("Abandon Audit") },
        { "saConDelivery.delRow",tr("&DelRow"),SLOT(sDelRow()), NULL, "DelRow", QPixmap(":/image/minus.png"), NULL,  true, tr("DelRow") },
        { "saConDelivery.print",tr("&Print"),SLOT(sPrint()), NULL, "Print", QPixmap(":/image/printer.png"), NULL,  true, tr("Print") },
        { "saConDelivery.choose",tr("&Choose"),SLOT(sChoose()), NULL, "Choose", QPixmap(":/image/page_accept.png"), NULL,  true, tr("Choose") },
        { "saConDelivery.first",tr("&First"),SLOT(sFirst()), NULL, "First", QPixmap(":/image/first48.png"), NULL,  true, tr("First") },
        { "saConDelivery.next",tr("&Next"),SLOT(sNext()), NULL, "Next", QPixmap(":/image/n-next48.png"), NULL,  true, tr("Next") },
        { "saConDelivery.previous",tr("&Previous"),SLOT(sPrevious()), NULL, "Previous", QPixmap(":/image/n-previous48.png"), NULL,  true, tr("Previous") },
        { "saConDelivery.end",tr("&End"),SLOT(sEnd()), NULL, "End", QPixmap(":/image/end48.png"), NULL,  true, tr("End") },
        { "saConDelivery.cancel",tr("&Cancel"),SLOT(sCancel()), NULL, "Cancel", QPixmap(":/image/cancel.png"), NULL,  true, tr("Cancel") }
    ...
    childItemProperties childItem[] = {
            ...
            { "orderNo","订单号","订单号", 8, 120,true, false },
    ...
    */
    ...
    QList<SaPriceBean *> proBeanList = SaPriceDAO::querySaPricee();
...
void SaConDeliverWin::sInvClick(){
...
void SaConDeliverWin::footResized(int logicalIndex, int oldSize, int newSize){
    mFootView->resizeSection(logicalIndex + 1,newSize);
}
void SaConDeliverWin::refreshData(){
...
void SaConDeliverWin::read(QString no){
    SaDispatchVouchBean *bean = SaDispatchDAO::getSaDispatchByNo(no);
    mNo = no;
    if(bean != NULL && bean->no().length() > 0){
        ...
        ui->addressEdit->setText(bean->shipAddress());
        ui->phoneEdit->setText(bean->phone());
        ui->linkEdit->setText(bean->linkman());
...
void SaConDeliverWin::inputClear(){
    WidgetBeanUtil::clearLayoutWidgetData(ui->headGridLayout);


}

void SaConDeliverWin::setInputEnable(bool b){
...
void SaConDeliverWin::addActionsTo(actionProperties acts[], unsigned int numElems){
...
void SaConDeliverWin::sFirst(){
    ...
    refreshData();
    QList<SaDispatchVouchBean *> vouchBeanList = SaDispatchDAO::querySaDispatchByPage(0,1,"03");
    if(vouchBeanList.size()>0){
        read(vouchBeanList.at(0)->no());
...
void SaConDeliverWin::sNext(){
    ...
    ui->tableWidget->clear();
    refreshData();
    QList<SaDispatchVouchBean *> vouchBeanList = SaDispatchDAO::querySaDispatchByPage(mRecord,1,"03");
    if(vouchBeanList.size()>0){
        read(vouchBeanList.at(0)->no());
    }
...
void SaConDeliverWin::sPrevious(){
    if(mIsEdit){
        if(QMessageBox::warning(this,"提示","是否放弃保存",QMessageBox::Yes | QMessageBox::No)==QMessageBox::Yes){
            mIsEdit = false;
            ui->toolBar->actions().at(0)->setEnabled(true);
...
void SaConDeliverWin::sCancel(){

    this->close();
}

void SaConDeliverWin::closeEvent(QCloseEvent *event){
...
void SaConDeliverWin::setupContextMenu(){
    setContextMenuPolicy(Qt::ActionsContextMenu);
}

void SaConDeliverWin::sNew(){
    inputClear();
    mIsEdit = true;
    mEditStatus = addFlag;
    setInputEnable(true);
    ...
    if(!mOverTop){
        ...
        for(int i = 0;i < rowNum; i++){
            ...
            for(int j = 0 ; j < stockList.size() ; j ++){
                ...
                if(stockList.at(j)->productCode() == puVouchsBean->productCode() ){
                    topSum = stockList.at(j)->canSum();
                    if(topSum-puVouchsBean->quantity()>=0){
                        isTop = false;
                        break ;
                    }
                }
            ...
            if(isTop){
                QMessageBox::information(this,"提示","此" + puVouchsBean->productName()
    ...
    if(mEditStatus == addFlag){
        ...
        if(mIsHathOrder){
            for(int i = 0;i < rowNum; i++){
                ...
                puVouchsBean->setDispatchNo(mNo);
                WidgetBeanUtil::setItemModelToBean(model,i,mChildItem,puVouchsBean);
                SaOrderVouchsBean *oBean = SaOrderDAO::getSaOrderVouchsBy(puVouchsBean->orderNo(),puVouchsBean->productCode());
                if(oBean && oBean->orderNo().length() > 2){
                    ...
                    if(real - puVouchsBean->quantity() < 0){
                        QMessageBox::warning(this,"提示","订单:"+ puVouchsBean->orderNo() + " ,商品："
                                             + puVouchsBean->productCode() + " 可发货数量不足！,最大发货数量为:"
                                             + QString::number(real));
                        return ;
                    }
        ...
        puVouchBean->setShipAddress(ui->addressEdit->text());
        puVouchBean->setPhone(ui->phoneEdit->text());
        puVouchBean->setLinkman(ui->linkEdit->text());
        ...
        if(mResult > 0 && cResult > 0 && DBManager::getDBManager()->getGoinDBConnection().commit()){
            ...
            setInputEnable(false);
            mCountRecord = SaDispatchDAO::getSaDispatchCount("01") - 1;
            mRecord = mCountRecord;
        ...
        if(mIsHathOrder){
            for(int i = 0; i < mChildVouchsList.size(); i++){
                SaOrderVouchsBean *oBean = SaOrderDAO::getSaOrderVouchsBy(mChildVouchsList.at(i)->orderNo(),mChildVouchsList.at(i)->productCode());
                if(oBean && oBean->orderNo().length() > 2){
                    double real = oBean->realQuantity();
                    //qDebug() << real << " quantity:" << mAddChildVouchsList.at(i)->quantity();
                    SaDispatchVouchsBean *aBean = SaDispatchDAO::getSaDispatchBy(mChildVouchsList.at(i)->dispatchNo(),mChildVouchsList.at(i)->productCode());
                    if(aBean && aBean->orderNo() == oBean->orderNo()){
                        real = real + aBean->quantity();
                    }
                    if(real - mChildVouchsList.at(i)->quantity() < 0){
                        QMessageBox::warning(this,"提示","订单:"+ mChildVouchsList.at(i)->orderNo() + " ,商品："
                                             + mChildVouchsList.at(i)->productCode() + " 可发货数量不足！,最大发货数量为:"
                                             + QString::number(real));
                        return ;
                    }
                    ...
                    QMessageBox::warning(this,"提示","订单:"+ mChildVouchsList.at(i)->orderNo() + " ,商品：" +
                                         mChildVouchsList.at(i)->productCode() + " 不存在");
                    return ;
                }
            }

            for(int i = 0; i < mAddChildVouchsList.size(); i++){
                SaOrderVouchsBean *oBean = SaOrderDAO::getSaOrderVouchsBy(mAddChildVouchsList.at(i)->orderNo(),mAddChildVouchsList.at(i)->productCode());
                if(oBean && oBean->orderNo().length() > 2){
                    double real = oBean->realQuantity();
                    //qDebug() << real << " quantity:" << mAddChildVouchsList.at(i)->quantity();
                    SaDispatchVouchsBean *aBean = SaDispatchDAO::getSaDispatchBy(mAddChildVouchsList.at(i)->dispatchNo(),mAddChildVouchsList.at(i)->productCode());
                    if(aBean && aBean->orderNo() == oBean->orderNo()){
                        real = real + aBean->quantity();
                    }
                    if(real - mAddChildVouchsList.at(i)->quantity() < 0){
                        QMessageBox::warning(this,"提示","订单:"+ mAddChildVouchsList.at(i)->orderNo() + " ,商品："+ mAddChildVouchsList.at(i)->productCode() + " 可发货数量不足！,最大发货数量为:" + QString::number(real));
                        return ;
                    }

                }else{
                    QMessageBox::warning(this,"提示","订单:"+ mAddChildVouchsList.at(i)->orderNo() + " ,商品：" +
                                         mAddChildVouchsList.at(i)->productCode() + " 不存在");
                    return ;
                }
    ...
    if(bean && bean->no().length() > 0){
        ...
        if(mIsHathOrder){
            for(int i=0;i<vouchsList.size();i++){
                SaOrderVouchsBean *oBean = SaOrderDAO::getSaOrderVouchsBy(vouchsList.at(i)->orderNo(),vouchsList.at(i)->productCode());
                double real = 0;
                if(oBean && oBean->orderNo().length() > 2 ){
                    real = oBean->quantity() - oBean->outQuantity();
                    if(real - vouchsList.at(i)->quantity() < 0){
                        QMessageBox::critical(this,"错误","订单:" + vouchsList.at(i)->orderNo() + " 商品："
                                              + vouchsList.at(i)->productName() + " 可发货数量为："
                                              + QString::number(real) );
                        return ;
                    }
                }else{
                    QMessageBox::critical(this,"错误","订单:" + vouchsList.at(i)->orderNo() + " 商品："
                                          + vouchsList.at(i)->productName() + " 不存在");
                    return ;
                }
        ...
        if(mIsMakeRd){
            RdBean *rdbean = RdDAO::getRdByBusNo(ui->noEdit->text(),HaoKeApp::getVouchRdSaOut());
            if(rdbean && rdbean->rdNo().length() > 0 ){
                QMessageBox::warning(this,"错误",rdbean->rdNo() + " 出库单已存在,不能重复生成,此单不能再审核！");
                return ;
            }
        }
        ...
        if(!mOverTop){
            ...
            for(int i = 0; i < vouchsList.size() ; i++){
                ...
                for(int j = 0 ; j < stockList.size() ; j ++){
                    ...
                    if(stockList.at(j)->productCode() == vouchsList.at(i)->productCode() ){
                        topSum = stockList.at(j)->sum();
                        if(topSum-vouchsList.at(i)->quantity()>=0){
                            isTop = false;
                            break ;
                        }
                    }
                ...
                if(isTop){
                    QMessageBox::information(this,"提示","此" + vouchsList.at(i)->productName()
                                             + " 库存不足,可发货量为:"+ QString::number(topSum) + " 不能保存");
                    return ;
                }
        ...
        if(mIsHathOrder){
            for(int i=0;i<vouchsList.size();i++){
                SaOrderVouchsBean *oBean = SaOrderDAO::getSaOrderVouchsBy(vouchsList.at(i)->orderNo(),vouchsList.at(i)->productCode());
                if(oBean && oBean->orderNo().length() > 2 ){
                    oBean->setOutQuantity(oBean->outQuantity() + vouchsList.at(i)->quantity());
                    if(oBean->quantity() == oBean->outQuantity())oBean->setIsComplete(true);
                    SaOrderDAO::updateSaOrderVouchs(oBean);
                    sResult = sResult + 1;
                }else{
                    DBManager::getDBManager()->getGoinDBConnection().rollback();

                    QMessageBox::critical(this,"错误","无此订单不能审核,订单号:" + vouchsList.at(i)->orderNo()  );
                    return ;
                }
            }
        }
        ...
        if(mIsMakeRd){
            ...
            rdbean->setVouchType(HaoKeApp::getVouchRdCtOut());
            ...
            if(ui->memoEdit->text().length() < 1)
                rdbean->setMemo("委托代销发货单自动生成,单号:" + bean->no());
            ...
            if(mResult > 0){
                ...
                DBManager::getDBManager()->getGoinDBConnection().rollback();
                QMessageBox::critical(this,"提示","自动生成出库单出错,不能保存");
                return;
            }
...
void SaConDeliverWin::sAbAudit(){
    ...
    if(bean !=NULL && bean->no().length() > 0){
        ...
        if(mIsHathOrder){
            for(int i=0;i<vouchsList.size();i++){
                ...
                if(oBean && oBean->orderNo().length() > 2 ){
                    ...
                    QMessageBox::critical(this,"错误","订单:" + vouchsList.at(i)->orderNo() + " 商品："
                                          + vouchsList.at(i)->productName() + " 不存");
                    return ;
                }
        ...
        //弃审核删除出库单
        if(mIsMakeRd){
            RdBean *rdbean = RdDAO::getRdByBusNo(ui->noEdit->text(),HaoKeApp::getVouchRdCtOut());
            if(rdbean && rdbean->rdNo().length() > 0 && rdbean->isHandle()){
                QMessageBox::warning(this,"提示",rdbean->rdNo() + " 出库单已审核，此单不能弃审核！");
                return ;
            }
...
void SaConDeliverWin::sPrint(){
    QString fileName = "ReportsaConDelivery.xml";
    QString title = "委托代销发货单";
    QString formatData = "";
...
void SaConDeliverWin::sChoose(){
    ...
    childItemProperties childItem[] = {
            ...
            { "quantity","总订货数量","总订货数量", 9, 80,true, true },
            { "not_quantity","未发货数量","未发货数量", 10, 80,true, true },
    ...
    QString sql = " SELECT a.id, a.no, a.bus_date, a.bus_type, a.cus_code, b.cus_name, a.wh_code,a.dep_code,a.demand_date,c.product_code,"
            ...
            "( SELECT g.*,g.quantity-g.out_quantity- COALESCE(h.sum_qunatity,0) as not_quantity "
                " from  sa_order_vouchs g "
                " LEFT JOIN ( "
                    " SELECT e.order_no,e.product_code,sum(quantity) as sum_qunatity "
                    " from sa_dispatch_vouchs e inner join sa_dispatch_vouch f ON e.dispatch_no=f.no "
                    " where f.is_handle=false AND f.bus_type='01'"
                    " GROUP BY e.order_no,e.product_code) h "
                " ON g.order_no=h.order_no AND g.product_code=h.product_code "
                " where (g.is_complete=FALSE OR g.is_complete is null) AND g.quantity-g.out_quantity-COALESCE(h.sum_qunatity,0) > 0 "
            ...
            " AND a.no=c.order_no AND a.is_handle=true AND c.product_code=d.product_code AND a.cus_code= b.cus_code "
            " ORDER BY id ASC";
...
void SaConDeliverWin::setValue(int &recNo, QString &paramName, QVariant &paramValue, int reportPage){
    Q_UNUSED(reportPage);
...
void SaConDeliverWin::on_tableWidget_itemChanged(QTableWidgetItem *item)
{
...
void SaConDeliverWin::on_cusSearchEditbox_editChanged(const QString &arg1)
{
    if(mIsEdit){
        CustomerBean *cusBean = CustomerDAO::getCusByCode(ui->cusSearchEditbox->text());
        if(cusBean && cusBean->cusCode().length() > 1){
            ui->addressEdit->setText(cusBean->cusAddress());
            ui->phoneEdit->setText(cusBean->phone());
            ui->linkEdit->setText(cusBean->contacts());
        }
    }
}
