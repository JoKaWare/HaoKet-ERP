...
SaDeliveryQueryForm::SaDeliveryQueryForm(QWidget *parent) :
    QueryForm(parent)
{
    QString lSql1 = "select e.*,d.emp_name from (select f.*,c.dep_name from "
            ...
            " WHERE a.bus_type='01' AND a.bus_date>=? AND a.bus_date<=? AND b.cus_type like ? AND a.cus_code like ? ) f "
    ...
    QString dSql1 = "SELECT f.*,j.dep_name,h.emp_name,w.wh_name from ( "
            " SELECT a.id, a.no, a.bus_date, a.bus_type, a.sa_type, a.cus_code, b.cus_name, a.wh_code,a.dep_code,c.order_no,c.product_code,"
                       ...
                       " WHERE a.bus_type='01' AND a.bus_date>=? AND a.bus_date<=? AND b.cus_type like ? AND a.cus_code like ? "
    ...
    QString dSql2 = "select f.*,b.cus_name,j.dep_name,h.emp_name,w.wh_name from ( "
            ...
            " WHERE a.no like ? AND a.bus_type='01' AND a.no=c.dispatch_no AND c.product_code=d.product_code) f "
...
void SaDeliveryQueryForm::sTableClicked(QString no){
    if(no.length()>1){
        if(!HaoKeApp::getInstance()->gui()->subWinIsOpen(tr("Sales Delivery"))){
            SaDeliveryWin *pWin = new SaDeliveryWin(no,this);
