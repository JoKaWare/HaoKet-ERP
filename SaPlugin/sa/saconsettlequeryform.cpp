#include "saconsettlequeryform.h"
...
SaConSettleQueryForm::SaConSettleQueryForm(QWidget *parent) :
    QueryForm(parent)
{
    QString lSql1 = "select e.*,d.emp_name from (select f.*,c.dep_name from "
            ...
            " WHERE a.bus_type='02' AND a.settle_date>=? AND a.settle_date<=? AND b.cus_type like ? AND a.cus_code like ? ) f "
    ...
    QString lSql2 = "select e.*,d.emp_name from (select f.*,c.dep_name from "
            " (select a.*,b.cus_name from sa_settle_vouch a "
            " left join ba_customer b on a.cus_code=b.cus_code "
            " WHERE a.bus_type='02' AND a.no like ? ) f"
    ...
    QString dSql1 =
            " SELECT a.id, a.no, a.settle_date,  a.settle_type, a.cus_code, b.cus_name,a.dep_code,c.rd_no,c.product_code,"
                       ...
                       " WHERE a.bus_type='02' AND a.settle_date>=? AND a.settle_date<=? AND b.cus_type like ? AND a.cus_code like ? "
    ...
    QString dSql2 = "select f.*,b.cus_name from ( "
            ...
            "  WHERE a.bus_type='02' AND a.no like ?  AND a.no=c.settle_no AND c.product_code=d.product_code) f "
...
void SaConSettleQueryForm::sTableClicked(QString no){
    if(no.length()>1){
        if(!HaoKeApp::getInstance()->gui()->subWinIsOpen(tr("Consignment Settle"))){
            SaConSettleWin *pWin = new SaConSettleWin(no,this);
