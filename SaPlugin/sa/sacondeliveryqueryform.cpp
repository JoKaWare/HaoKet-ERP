#include "sacondeliveryqueryform.h"
...
SaConDeliveryQueryForm::SaConDeliveryQueryForm(QWidget *parent) :
    QueryForm(parent)
{
    QString lSql1 = "select e.*,d.emp_name from (select f.*,c.dep_name from "
            ...
            " WHERE a.bus_type='03' AND a.bus_date>=? AND a.bus_date<=? AND b.cus_type like ? AND a.cus_code like ? ) f "
    ...
    QString lSql2 = "select e.*,d.emp_name from (select f.*,c.dep_name from "
            " (select a.*,b.cus_name from sa_dispatch_vouch a "
            " left join ba_customer b on a.cus_code=b.cus_code "
            " WHERE a.no like ? AND a.bus_type='03' ) f"
    ...
    QString dSql1 = "SELECT f.*,j.dep_name,h.emp_name,w.wh_name from ( "
            " SELECT a.id, a.no, a.bus_date, a.bus_type, a.sa_type, a.cus_code, b.cus_name, a.wh_code,a.dep_code,c.order_no,c.product_code,"
                       ...
                       " WHERE a.bus_type='03' AND a.bus_date>=? AND a.bus_date<=? AND b.cus_type like ? AND a.cus_code like ? "
    ...
    QString dSql2 = "select f.*,b.cus_name,j.dep_name,h.emp_name,w.wh_name from ( "
            ...
            " WHERE a.no like ? AND a.bus_type='03' AND a.no=c.dispatch_no AND c.product_code=d.product_code) f "
    ...
    childItemProperties childItem[] = {
            ...
            { "unit","委托代销单位","委托代销单位", 12,60, true, false },
            { "quantity","数量","数量", 13, 60,true, true },
            { "price","委托代销单价","委托代销单价", 14,80, true, true },
            { "money","委托代销金额","委托代销金额", 15,80, true, false }
    ...
    QString formId = HaoKeApp::saSubId() + SaPlugin::conDeliverylQueryWinId();
    QString winTitle = "委托代销发货单查询";
    HaoKeApp::writeSysLog(HaoKeApp::getInstance()->getAccountId() ,winTitle,HaoKeApp::saSubId().toInt(),formId.toInt(),0,"");

    setFormId(formId);
    setTabListText("委托代销发货单列表");
    setTabDetailText("委托代销发货明细");
    //设置明细合计列号
    QList<int > totalItem;
    totalItem << 13 << 15;
...
void SaConDeliveryQueryForm::sTableClicked(QString no){
    if(no.length()>1){
        if(!HaoKeApp::getInstance()->gui()->subWinIsOpen(tr("Consignment Delivery"))){
            SaConDeliverWin *pWin = new SaConDeliverWin(no,this);
