#include "sadeliverywin.h"
#include "ui_sadeliverywin.h"
#include "haokeapp.h"
...
SaDeliveryWin::SaDeliveryWin(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::SaDeliveryWin)
{
    ui->setupUi(this);
    init();
    ...
    QList<SaDispatchVouchBean *> vouchBeanList = SaDispatchDAO::querySaDispatchByPage(mCountRecord,1,"01");
...
SaDeliveryWin::SaDeliveryWin(QString no, QWidget *parent):
    QMainWindow(parent),
    ui(new Ui::SaDeliveryWin)
{
    ui->setupUi(this);
...
SaDeliveryWin::~SaDeliveryWin()
{
    delete ui;
}

void SaDeliveryWin::readAuthority(){
    LimitsCommon::authority(ui->toolBar,mWinId.toInt());
}
void SaDeliveryWin::init(){
    QApplication::setOverrideCursor(Qt::WaitCursor);
    //窗体ID号
    mWinId = HaoKeApp::saSubId() + SaPlugin::deliverylWinId();
    mWinTitle = "销售发货单";
    ...
    actionProperties actsBase[]= {
        { "saDelivery.new",tr("&New"),SLOT(sNew()), NULL, "New", QPixmap(":/image/Add.png"), NULL,  true, tr("New") },
        { "saDelivery.edit",tr("&Edit"),SLOT(sEdit()), NULL, "Edit", QPixmap(":/image/edit.png"), NULL,  true, tr("Edit") },
        { "saDelivery.delete",tr("&Delete"),SLOT(sDelete()), NULL, "Delete", QPixmap(":/image/cross_48.png"), NULL,  true, tr("Delete") },
        { "saDelivery.save",tr("&Save"),SLOT(sSave()), NULL, "Save", QPixmap(":/image/save.png"), NULL,  true, tr("Save") },
        { "saDelivery.audit",tr("&Audit"),SLOT(sAudit()), NULL, "Audit", QPixmap(":/image/stamp_48.png"), NULL,  true, tr("Audit") },
        { "saDelivery.abandonAudit",tr("&Abandon Audit"),SLOT(sAbAudit()), NULL, "Abandon Audit", QPixmap(":/image/c_stamp_48.png"), NULL,  true, tr("Abandon Audit") },
        { "saDelivery.delRow",tr("&DelRow"),SLOT(sDelRow()), NULL, "DelRow", QPixmap(":/image/minus.png"), NULL,  true, tr("DelRow") },
        { "saDelivery.print",tr("&Print"),SLOT(sPrint()), NULL, "Print", QPixmap(":/image/printer.png"), NULL,  true, tr("Print") },
        { "saDelivery.choose",tr("&Choose"),SLOT(sChoose()), NULL, "Choose", QPixmap(":/image/page_accept.png"), NULL,  true, tr("Choose") },
        { "saDelivery.first",tr("&First"),SLOT(sFirst()), NULL, "First", QPixmap(":/image/first48.png"), NULL,  true, tr("First") },
        { "saDelivery.next",tr("&Next"),SLOT(sNext()), NULL, "Next", QPixmap(":/image/n-next48.png"), NULL,  true, tr("Next") },
        { "saDelivery.previous",tr("&Previous"),SLOT(sPrevious()), NULL, "Previous", QPixmap(":/image/n-previous48.png"), NULL,  true, tr("Previous") },
        { "saDelivery.end",tr("&End"),SLOT(sEnd()), NULL, "End", QPixmap(":/image/end48.png"), NULL,  true, tr("End") },
        { "saDelivery.cancel",tr("&Cancel"),SLOT(sCancel()), NULL, "Cancel", QPixmap(":/image/cancel.png"), NULL,  true, tr("Cancel") }
...
void SaDeliveryWin::footResized(int logicalIndex, int oldSize, int newSize){
    mFootView->resizeSection(logicalIndex + 1,newSize);
}
void SaDeliveryWin::refreshData(){
...
void SaDeliveryWin::read(QString no){
    SaDispatchVouchBean *bean = SaDispatchDAO::getSaDispatchByNo(no);
    mNo = no;
    if(bean != NULL && bean->no().length() > 0){
...
void SaDeliveryWin::inputClear(){
    WidgetBeanUtil::clearLayoutWidgetData(ui->headGridLayout);


}

void SaDeliveryWin::setInputEnable(bool b){
...
void SaDeliveryWin::addActionsTo(actionProperties acts[], unsigned int numElems){
...
void SaDeliveryWin::sFirst(){
    ...
    refreshData();
    QList<SaDispatchVouchBean *> vouchBeanList = SaDispatchDAO::querySaDispatchByPage(0,1,"01");
    if(vouchBeanList.size()>0){
        read(vouchBeanList.at(0)->no());
...
void SaDeliveryWin::sNext(){
    ...
    ui->tableWidget->clear();
    refreshData();
    QList<SaDispatchVouchBean *> vouchBeanList = SaDispatchDAO::querySaDispatchByPage(mRecord,1,"01");
    if(vouchBeanList.size()>0){
        read(vouchBeanList.at(0)->no());
    }
...
void SaDeliveryWin::sCancel(){

    this->close();
}

void SaDeliveryWin::closeEvent(QCloseEvent *event){
...
void SaDeliveryWin::setupContextMenu(){
    setContextMenuPolicy(Qt::ActionsContextMenu);
}

void SaDeliveryWin::sNew(){
    inputClear();
    mIsEdit = true;
    mEditStatus = addFlag;
    setInputEnable(true);
    ...
    mNo = ItemUtil::getToDayMaxNo(mWinId,"sa_dispatch_vouch",QDate::currentDate(),nCount);
...
void SaDeliveryWin::sAudit(){
    ...
    if(bean && bean->no().length() > 0){
        ...
        if(mIsMakeRd){
            ...
            rdbean->setVouchType(HaoKeApp::getVouchRdSaOut());
            ...
            if(ui->memoEdit->text().length() < 1)
                rdbean->setMemo("发货单自动生成,单号:" + bean->no());
...
void SaDeliveryWin::sPrint(){
    QString fileName = "ReportsaDelivery.xml";
    QString title = "销售发货单";
    QString formatData = "";
...
void SaDeliveryWin::setValue(int &recNo, QString &paramName, QVariant &paramValue, int reportPage){
    Q_UNUSED(reportPage);
...
void SaDeliveryWin::on_tableWidget_itemChanged(QTableWidgetItem *item)
{
...
void SaDeliveryWin::on_cusSearchEditbox_editChanged(const QString &arg1)
{
    if(mIsEdit){
