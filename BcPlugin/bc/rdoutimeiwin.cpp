...
void RdOutImeiWin::init(){
    ...
    if(mVouchType == HaoKeApp::getVouchRdSaOut()
        ...
        ui->label_16->setText("<html><head/><body><p><span style=\" color:#ff0000;\">其他出库</span></p></body></html>");
        ...
        ui->label_16->setText("<html><head/><body><p><span style=\" color:#ff0000;\">采购退货</span></p></body></html>");
        ...
        ui->label_3->setText("供 应 商:");
        ui->arrivalLabel->setText("退货单号:");

    }else if(mVouchType == HaoKeApp::getVouchRdCtOut()){
        //11委托代销出库单
        ui->label_16->setText("<html><head/><body><p><span >委托代销出库单</span></p></body></html>");
        QList<CustomerBean *> cusBeanList = CustomerDAO::queryCus();
        QStringList cusList;
        for(int i =0 ; i < cusBeanList.size(); i++){
            cusList << cusBeanList.at(i)->cusCode() + " - " +cusBeanList.at(i)->cusName();
    ...
    if(bean !=NULL && bean->rdNo().length() > 0){
        ...
        for(int i = 0 ; i < imeiList.size(); i ++ ){
            ...
            if(c < 1){
                DBManager::getDBManager()->getGoinDBConnection().rollback();
                QApplication::restoreOverrideCursor();
                QMessageBox::information(this,"提示","弃审出错！IMEI:" + imeiList.at(i)->imei());
...
void RdOutImeiWin::on_saveButton_clicked()
{
    ...
    for(int i = 0; i < rowNum; i++ ){
        ...
        if(count < 1){
            ...
            QMessageBox::information(this,"提示","IMEI:" + imei + "未在库存，不能保存！");
            return ;
        }
        ...
        if(imeiBean && imeiBean->whCode() != ui->whSearchEditbox->text().trimmed()){
            ui->tableWidget2->setCurrentCell(i,0);
            QMessageBox::information(this,"提示","IMEI:" + imei + "未在仓库:" + ui->whSearchEditbox->subText() +"中，不能保存！");
            return ;
        }
...
void RdOutImeiWin::on_tableWidget2_itemChanged(QTableWidgetItem *item)
{
    if(item->column() == 0 && mEditStatus == editFlag && mIsEditMode && !mIsImport){
        ...
        int count = StImeiDAO::getImeiCountByImei(imei);
        if(count < 1){
            QMessageBox::information(this,"提示","IMEI:" + imei + "未在库存，不能录入！");
            return ;
        }
        ...
        if(imeiBean && imeiBean->whCode() != ui->whSearchEditbox->text().trimmed()){
            QMessageBox::information(this,"提示","IMEI:" + imei + "未在仓库:" + ui->whSearchEditbox->subText() +"中，不能录入！");
            return ;
        }
