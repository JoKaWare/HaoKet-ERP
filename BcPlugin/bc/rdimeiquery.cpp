...
RdImeiQuery::RdImeiQuery(QWidget *parent) :
    SingleQueryForm(parent)
{
    QString dSql1 =
            " SELECT a.*,b.product_name,c.supplie_name,d.wh_name "
            " FROM rd_in_imei a "
            ...
            " WHERE vouch_type='01' AND bus_date>=? AND bus_date<=? "
            " AND a.bus_code LIKE ? AND b.type_id LIKE ? AND a.product_code LIKE ?";
    QString dSql2 = " SELECT a.*,b.product_name,c.supplie_name,d.wh_name "
            " FROM rd_in_imei a "
            " INNER JOIN ba_product b ON a.product_code=b.product_code "
            " LEFT JOIN ba_suppliers c ON a.bus_code=c.supplie_code "
            " LEFT JOIN ba_ware_house d ON a.wh_code=d.wh_code "
...
RdImeiQuery::RdImeiQuery(QString vouch_type, QWidget *parent) :
    SingleQueryForm(parent)
{
    QString dSql1 =
            " SELECT a.*,b.product_name,c.supplie_name,d.wh_name "
    ...
    if(vouch_type == HaoKeApp::getVouchRdSaRe()
            || vouch_type == HaoKeApp::getVouchRdCtRe()){
        dSql1 = " SELECT a.*,b.product_name,c.cus_name,d.wh_name "
                " FROM rd_in_imei a "
                " INNER JOIN ba_product b ON a.product_code=b.product_code "
                " LEFT JOIN ba_customer c ON a.bus_code=c.cus_code "
                " LEFT JOIN ba_ware_house d ON a.wh_code=d.wh_code "
                " WHERE (vouch_type='12' OR vouch_type='15') AND bus_date>=? AND bus_date<=? "
                " AND a.bus_code LIKE ? AND b.type_id LIKE ? AND a.product_code LIKE ?";
...
void RdImeiQuery::sTableClicked(QString no){
    if(no.length()>1){
        if(!HaoKeApp::getInstance()->gui()->subWinIsOpen("串码入库")){
            RdInImeiWin *pWin = new RdInImeiWin(no,this);
            pWin->setWindowTitle("串码入库");
